...
import { Calendar as CalendarIcon, ClipboardMinus, Loader2, Package, Save, UserCheck, X } from "lucide-react";
...
interface GlobalUsageModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess: () => void;
...
export function GlobalUsageModal({ isOpen, onClose, onSuccess, stockItems }: GlobalUsageModalProps) {
    ...
    const onSubmit = async (data: UsageFormValues) => {
        try {
            const res = await fetch(`/api/stock/${data.stockItemId}/usages`, {
                method: "POST",
            ...
            toast({
                title: "บันทึกการเบิกใช้สำเร็จ",
                description: "เพิ่มรายการเบิกใช้วัตถุดิบเรียบร้อยแล้ว",
            ...
            <DialogContent className="sm:max-w-[640px] bg-white">
                <DialogHeader>
                    <DialogTitle className="text-xl font-bold text-gray-800 flex items-center gap-2">
                        <ClipboardMinus className="w-5 h-5 text-purple-600" />
                        เบิกใช้วัตถุดิบ
                    </DialogTitle>
                </DialogHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <FormItem>
                                <FormLabel className="text-gray-700 font-medium">คงเหลือปัจจุบัน</FormLabel>
                                <Input
                                    value={selectedStockItem ? `${Number(selectedStockItem.currentQty).toLocaleString()} ${selectedStockItem.unit}` : "-"}
                                    disabled
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-gray-700 font-medium">จำนวนที่เบิก</FormLabel>
                                        <FormControl>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-gray-700 font-medium">วันที่เบิก</FormLabel>
                                        <Popover>
