...
import { Plus, Eye, Trash2, Search, FileSpreadsheet, RefreshCw, Factory, PlayCircle, Clock } from "lucide-react";
...
export default function ProductionPage() {
    ...
    const fetchAvailablePOs = async () => {
        try {
            ...
            const data = await res.json();
            const available = data.filter((po: PurchaseOrder) => !po.productionOrder);
    ...
    useEffect(() => {
        ...
        if (filterSearch) {
            result = result.filter(prod => 
                ...
                formatPoNumber(prod.purchaseOrder.poNumber).toLowerCase().includes(filterSearch.toLowerCase()) ||
                prod.purchaseOrder.supplier.companyName.toLowerCase().includes(filterSearch.toLowerCase())
            );
        }
    ...
    const handleCreateProduction = async () => {
        ...
        try {
            const res = await fetch("/api/production-orders", {
                ...
                body: JSON.stringify({ purchaseOrderId: selectedPO }),
            });
                            ...
                            <div key={prod.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden border border-gray-200 flex flex-col group">
                                ...
                                <div className="p-4 space-y-3 flex-1">
                                    <div className="bg-violet-50 p-2 rounded-lg border border-violet-100">
                                        ...
                                        <span className="font-bold text-gray-800 text-sm">{formatPoNumber(prod.purchaseOrder.poNumber)}</span>
                                    </div>
                                    ...
                                    <div className="border-t border-gray-100 pt-2 mt-2">
                                        <span className="text-gray-400 text-[10px] font-bold uppercase block mb-1">รายการสินค้า</span>
            ...
            <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
                <DialogContent className="bg-white">
                    ...
                    <div className="space-y-4 py-4">
                        <div>
                            <label className="text-sm font-medium mb-2 block">เลือก PO (Select Purchase Order)</label>
                                ...
                                <SelectContent>
                                    {availablePOs.map(po => (
                                        <SelectItem key={po.id} value={po.id}>
                                            {formatPoNumber(po.poNumber)} - {po.supplier.companyName}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
